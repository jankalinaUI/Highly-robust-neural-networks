...
class TuneModel:
    ...
    def cross_val(self,K,epochs, model, file,alpha,batch_size):
        ...
        for train,test in KFold_split.split(self.train_x):
            ...
            if model == 'RBF':
                ...
                RBF.fit(train_x_cv,train_y_cv,epochs = self.epochs,verbose = 0,batch_size = self.batch)
                ...
                predsTMLP = net_train.IQR_QMLP(epochs = self.epochs,batch_size = self.batch)
                ...
                RBF.fit(train_x_cv,train_y_cv,epochs = self.epochs,verbose = 0,batch_size = self.batch)
                MLP.fit(train_x_cv,train_y_cv,epochs = self.epochs,verbose = 0,batch_size = self.batch)
                predsMLP = MLP.predict(test_x_cv)
                predsRBF = RBF.predict(test_x_cv)
