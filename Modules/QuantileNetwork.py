...
from RBFLayer_keras import RBFLayer, InitCentersRandom
import matplotlib.pyplot as plt
from keras.datasets import boston_housing
from keras.wrappers.scikit_learn import KerasRegressor
#from Losses_keras import psi,quantile_nonlinear,least_weighted_square
...
class QuantileNetwork:
  ...
  def MLP_model(self,neurons1 = None,neurons2= None,input_shape = None, loss = None):
    ...
    layer = tf.keras.layers.Dense(self.neurons1, activation = tf.keras.activations.sigmoid)(inputs)
    lay = tf.keras.layers.Dense(self.neurons2,activation = tf.keras.activations.sigmoid)(layer)
    out = tf.keras.layers.Dense(1)(lay)
    

    model = tf.keras.models.Model(inputs = inputs , outputs = out)

    model.compile(loss = self.loss, optimizer = tf.keras.optimizers.RMSprop())
  ...
  def RBF_model(self,x, input_shape, units, betas, loss):
    ...
    rbf = rbflayer(inputs)
    out = tf.keras.layers.Dense(1)(rbf)
